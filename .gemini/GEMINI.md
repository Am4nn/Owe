...
**No Web Support:** Owe is strictly a mobile-only application targeting iOS and Android natively.
- Do not add, maintain, or suggest `Platform.OS === 'web'` or `Platform.OS !== 'web'` checks.
- Do not configure web build targets in `app.json` or `package.json`.
- All features must be built exclusively for native execution.
