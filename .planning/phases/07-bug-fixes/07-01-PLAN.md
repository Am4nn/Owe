...
### BUG-12 — Alert.prompt crashes on Android and web ✅ MODAL CONFIRMED (send blocked by BUG-12b)
...
### BUG-12 — "Invite by email" modal send: permission denied for table users 🔴 OPEN
**Symptom:** After BUG-12 modal fix, tapping "Send Invite" returns `Error: permission denied
for table users`.
**Root cause:** Under investigation — `group_invites` INSERT likely joins `auth.users` to look
up the invited email, and the `authenticated` role lacks SELECT on `auth.users` (which is
Supabase-managed and not in the public schema).
**Status:** OPEN

---

### BUG-14 — Leave group "nothing happens" on web 🔧 FIX APPLIED — pending verification
...
### BUG-13 — Leave group FK violation ✅ CONFIRMED FIXED (user reported)
...
13. ✅ Leave group succeeds without FK violation
