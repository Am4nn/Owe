...
Purpose: The expense entry flow is the core of the app — everything else (balances, settlement) depends on expenses existing. Plan 02-01 establishes the data model and UI entry point that plans 02-02 and 02-03 build on.
...
// getCurrentUserId(): async helper — supabase.auth.getUser().data.user.id
```
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - computeEqualSplits(1000, ['a','b','c']) → [{member_id:'a',amount_cents:334},{member_id:'b',amount_cents:333},{member_id:'c',amount_cents:333}]
    - computeEqualSplits(100, ['a','b']) → [{amount_cents:50},{amount_cents:50}] (no remainder)
    - computeSharesSplits(1000, [{member_id:'a',shares:1},{member_id:'b',shares:2}]) → [{amount_cents:333},{amount_cents:667}] (total = 1000)
    - computePercentageSplits(1000, [{member_id:'a',percentage:33},{member_id:'b',percentage:67}]) → [{amount_cents:330},{amount_cents:670}]
...
1. Write migration `supabase/migrations/20260228000003_expense_loop.sql`. Include:
   ...
   - SECURITY NOTE: Follow foundation pattern — ENABLE ROW LEVEL SECURITY immediately after each CREATE TABLE before any other statement.
...
4. Create `src/features/expenses/categories.ts`. Export a static `CATEGORIES` array:
```typescript
...
```
  ...
  <verify>
    <automated>cd "/c/Aman/Coding-Bamzii/Web Development/Owe" && npx jest --testPathPattern="expenses/splits" --passWithNoTests 2>&1 | tail -5</automated>
  </verify>
  <done>
    ...
    - All splits.test.ts cases pass (or test file created if missing and passes)
  </done>
</task>
...
2. Create `src/components/expenses/SplitEditor.tsx`. Props: `members: GroupMember[]`, `totalCents: number`, `splitType: SplitType`, `onSplitTypeChange: (type: SplitType) => void`, `onSplitsChange: (splits: SplitInput[]) => void`. Renders:
   ...
   - For Exact mode: shows a TextInput per member for cents entry. Displays running total vs target. Validation border turns `border-red-500` when sum ≠ totalCents.
...
8. Update `app/(app)/index.tsx`. Add: import and render `<ExpandableFAB />` replacing the existing simple FAB. Filter groups to exclude `is_direct = true` groups (update useGroups query OR add filter here: `groups.filter(g => !g.is_direct)`). Group type needs `is_direct?: boolean` — update `src/features/groups/types.ts` to add `is_direct: boolean` field to the `Group` interface.
  </action>
  <verify>
...
3. Navigation: Dashboard FAB expands → Manual Entry → expense form renders → submit creates expense → appears in group screen
