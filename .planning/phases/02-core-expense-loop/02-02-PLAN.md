...
<objective>
Build the balance layer: dashboard totals (BALS-01), per-group member balance breakdown (BALS-02), and the debt-simplify Supabase Edge Function with greedy algorithm (BALS-03). Wire Supabase Realtime to invalidate balances when expenses change. Also add the Activity and Settlement History navigation entries to the group detail screen (sole owner of groups/[id]/index.tsx).
...
Output: Dashboard balance summary, group balances screen, simplified debts screen, Realtime sync, the deployed Edge Function, and navigation links to Activity and Settlement History screens (built in 02-03).
</objective>
...
// NOTE: Supabase PostgREST requires filtering joined tables by original table name, not alias
...
<task type="auto">
  <name>Task 2: Balance UI — dashboard summary, group detail screen (balances + Activity + Settlement History nav), simplified debts screen</name>
  <files>
    src/components/balances/BalanceCard.tsx,
    app/(app)/index.tsx,
    app/(app)/groups/[id]/index.tsx,
...
3. Update `app/(app)/groups/[id]/index.tsx`. This plan is the sole owner of this file. Add all group detail screen extensions here:
   ...
   - Add an "Activity" button (outlined, brand-primary border) that navigates to `/(app)/groups/${groupId}/activity`. Place it next to or below the "Simplified Debts" button in the same action row. This link is required so the Activity screen (built in 02-03) is reachable.
   - Add a "Settlement History" button (outlined, brand-primary border) that navigates to `/(app)/groups/${groupId}/settlements`. Place it in the same action row as the other navigation buttons. This link is required so the Settlement History screen (built in 02-03) is reachable.
...
4. Create `app/(app)/groups/[id]/balances.tsx`. Simplified debts screen:
   ...
   - FlatList of debt suggestions. Each row: `{from_display_name} pays {to_display_name} ${amount_cents/100 toFixed 2}`. Style: white text on dark-surface card with brand-primary "Settle" button on the right that navigates to settlement creation (settlement flow built in Plan 02-03 — button can be a placeholder for now: `router.push({ pathname: '/(app)/settlement/new', params: { payer_member_id: s.from_member_id, payee_member_id: s.to_member_id, amount_cents: s.amount_cents, group_id: groupId } })`).
  ...
  <done>
    ...
    - Group detail screen has "Simplified Debts", "Activity", and "Settlement History" navigation buttons — all three routes are reachable
...
5. Activity screen navigable from group detail via "Activity" button
6. Settlement History screen navigable from group detail via "Settlement History" button
7. Realtime: open app in two simulators; adding expense in one updates balances in the other within ~2 seconds
8. Edge Function exists at `supabase/functions/simplify-debts/index.ts` — no `SERVICE_ROLE` or `service_role` string in the file
</verification>
...
- Group detail screen has navigation buttons for Simplified Debts, Activity, and Settlement History (screens built in 02-03)
