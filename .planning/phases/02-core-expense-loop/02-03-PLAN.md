...
// mutationKey: ['expenses', 'create'] — MUST match setMutationDefaults in _layout.tsx
```
...
Offline pattern (OFFL-02) — critical implementation notes:
```typescript
...
//    — mutations created while offline are automatically paused and serialized to persister
```
...
```typescript
// Extract as standalone function (required for setMutationDefaults — function ref must survive serialization)
export async function createExpenseMutationFn(input: CreateExpenseInput): Promise<Expense> {
...
**BEFORE writing**: Check how `queryClient` is currently instantiated in `app/_layout.tsx`. If it's created inline (e.g. `const queryClient = new QueryClient(...)` inside the component or at module top of `_layout.tsx`), extract it to `src/lib/queryClient.ts` as a named export so it can be imported here at module scope without a circular reference. If it's already exported from `@/lib/queryClient`, use that import directly.

```typescript
import NetInfo from '@react-native-community/netinfo'
import { onlineManager } from '@tanstack/react-query'
import { queryClient } from '@/lib/queryClient'  // verify this path — extract if needed (see note above)
...
**useCreateSettlement():** mutationKey `['settlements', 'create']`. Implementation steps:
1. INSERT into settlements table with all fields from `CreateSettlementInput` (payer_id = payer_member_id, payee_id = payee_member_id, settled_at = new Date().toISOString()).
2. Resolve `actor_id` for the activity row — the activities table stores member_id, not user_id. Look it up: `const { data: actorMember } = await supabase.from('group_members').select('id').eq('group_id', input.group_id).eq('user_id', (await supabase.auth.getUser()).data.user!.id).single()` — use `actorMember.id` as actor_id.
3. INSERT activity row: `{ action_type: 'settlement_recorded', group_id: input.group_id, actor_id: actorMember.id, expense_id: null, metadata: { amount_cents: input.amount_cents } }`.
4. On success: invalidate `['settlements', input.group_id]`, `['balances']`, `['activity', input.group_id]`.
...
**useActivityFeed(groupId?: string):** queryKey `['activity', groupId ?? 'all']`, staleTime 30_000. Two execution paths:

- **With groupId** (filtered view — ACTY-02): `supabase.from('activities').select('*, actor:profiles!actor_id(display_name)').eq('group_id', groupId).order('created_at', { ascending: false }).limit(100)`

- **Without groupId** (all-groups view): Supabase PostgREST cannot express a subquery `WHERE group_id IN (SELECT ...)` directly via the JS client. Use a two-step approach: (1) fetch the current user's group_ids: `const { data: memberships } = await supabase.from('group_members').select('group_id').eq('user_id', userId)` — extract `groupIds = memberships.map(m => m.group_id)`. (2) Fetch activities: `supabase.from('activities').select('*, actor:profiles!actor_id(display_name)').in('group_id', groupIds).order('created_at', { ascending: false }).limit(100)`. If `groupIds` is empty, return `[]` immediately (skip the second query).

Join actor profile display_name via select with FK expansion as shown above. Map `actor.display_name` → `actor_display_name` on each item. Return `ActivityItem[]`.
...
4. Create `app/(app)/groups/[id]/activity.tsx`. Activity feed screen for a group:
   ...
   - FlatList of activity items. Each item renders:
     ...
     - relative time (created_at formatted as "2h ago", "yesterday", etc. — use simple logic: if < 60min show "Xm ago", if < 24h show "Xh ago", else show date)
...
5. Create `app/(app)/groups/[id]/settlements.tsx`. Settlement history screen for a group (SETL-03):
   ...
   - `useLocalSearchParams()` for groupId
   - Call `useSettlementHistory(groupId)` — returns settlements enriched with payer_display_name and payee_display_name
...
onPress={() => {
  ...
  router.push({
    ...
    // payer/payee pre-fill requires knowing current user's member_id — leave blank for manual pick on settlement form
  })
}}

// Left swipe → Remind (Phase 3 push notifications)
onPress={() => {
  swipeRef.current?.close()
...
4. Settlement flow: open settlement/new → fill form → submit → success screen fires confetti + haptic
5. Settlement history: after recording a settlement, navigate to groups/[id]/settlements — past settlement row appears with payer name, payee name, amount, date
6. Activity feed: after adding expense, activity screen for that group shows "added an expense" row
7. Comments: add comment on expense detail → appears in list below
8. Reactions: tap emoji on expense detail → reaction chip appears; tap again → removes
9. Offline test: enable airplane mode → add expense via form → enable network → expense syncs to Supabase
</verification>
