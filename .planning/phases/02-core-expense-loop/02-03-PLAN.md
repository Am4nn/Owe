...
must_haves:
  truths:
    ...
    - "User can view all past settlements for a group"
...
<objective>
Build settlement recording with confetti, the activity feed with comments and reactions, and the OFFL-02 offline mutation queue. Also wires the swipe-to-settle gesture on expense cards to navigate to the settlement form.

Purpose: This plan completes Phase 2 by closing the debt loop (settlement), providing social proof (activity feed), and making the app resilient offline. Together with 02-01 and 02-02, this delivers the full core product.
Output: Settlement flow, confetti screen, activity feed, comments, reactions, offline mutation queue via NetInfo + resumePausedMutations.
</objective>
...
**useSettlementHistory(groupId: string):** queryKey `['settlements', groupId]`, staleTime 30_000. Fetch settlements WHERE group_id = groupId ORDER BY settled_at DESC.
...
<task type="auto">
  <name>Task 2: Settlement screens + confetti + activity feed + comments + reactions + swipe wire-up</name>
  <files>
    src/components/settlement/ConfettiScreen.tsx,
    app/(app)/settlement/new.tsx,
    app/(app)/settlement/success.tsx,
...
4. Create `app/(app)/groups/[id]/activity.tsx`. Activity feed screen for a group:
   ...
   - Add link from group detail screen (`app/(app)/groups/[id]/index.tsx`) — add "Activity" tab/button navigating to this screen. (Update the groups/[id]/index.tsx to add an "Activity" button in the header or below the balances section.)

5. Update `app/(app)/expenses/[id]/index.tsx` (extended from Plan 02-01 base). Add below the expense details:
...
6. Update `src/components/expenses/ExpenseCard.tsx` swipe actions (from Plan 02-01). Wire right swipe Settle action to navigate to the settlement form:
```typescript
// Right swipe → Settle
onPress={() => {
...
5. Activity feed: after adding expense, activity screen for that group shows "added an expense" row
6. Comments: add comment on expense detail → appears in list below
7. Reactions: tap emoji on expense detail → reaction chip appears; tap again → removes
8. Offline test: enable airplane mode → add expense via form → enable network → expense syncs to Supabase
</verification>

<success_criteria>
- Settlement can be recorded from the settlement form; success shows full-screen confetti with haptic feedback
- Settlement history is viewable per group
