...
- [ ] `supabase db push` applies migration — `email_sent_at` column exists on `group_invites`
- [ ] `supabase functions deploy send-invite-email --no-verify-jwt` succeeds
- [ ] `supabase secrets set RESEND_API_KEY=<key> RESEND_FROM="Owe <invites@resend.dev>"` set in Vault
- [ ] DB Webhook created in Supabase Dashboard (group_invites → INSERT → Edge Function URL)
- [ ] App: tap **Invite Member** → enter a real email → tap **Send**
- [ ] Email arrives within ~5 seconds with correct group name + inviter name
- [ ] `group_invites.email_sent_at` is non-null after send
- [ ] `supabase functions logs send-invite-email --tail` shows `{ sent: 1 }` with no 500s
- [ ] Second invite to same email (conflict on UNIQUE constraint) is blocked at DB level — no duplicate send path
