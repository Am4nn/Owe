...
import { useExpense, useUpdateExpense, useDeleteExpense } from '@/features/expenses/hooks'
import { SplitEditor } from '@/components/expenses/SplitEditor'
import { CATEGORIES } from '@/features/expenses/categories'
import type { SplitType, SplitInput, Expense } from '@/features/expenses/types'
...
function EditExpenseForm({ expense, groupData }: { expense: Expense, groupData: { members: GroupMember[] } }) {
  const updateExpense = useUpdateExpense()
  const deleteExpense = useDeleteExpense()
  const { isPending: isDeleting } = deleteExpense
  ...
  async function onSubmit(values: FormValues) {
    try {
      ...
      showAlert('Error', err instanceof Error ? err.message : 'Failed to update expense')
    }
  }
...
export default function EditExpenseScreen() {
  ...
  const groupQuery = useGroup(expense?.group_id ?? '')

  return (
    <QueryGuard query={expenseQuery}>
      {(expense) => (
        <QueryGuard query={groupQuery}>
          {(groupData) => (
            <EditExpenseForm expense={expense} groupData={groupData} />
          )}
        </QueryGuard>
      )}
    </QueryGuard>
